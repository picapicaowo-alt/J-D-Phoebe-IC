...
export default async function KnowledgePage({
  ...
  searchParams: Promise<{ q?: string; layer?: KnowledgeLayer | "ALL"; projectId?: string; authorId?: string; tag?: string }>;
}) {
  ...
  const [assets, deletedAssets, projects, authors] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Knowledge Observatory</h1>
        <p className="mt-1 text-sm text-[hsl(var(--muted))]">Four-layer knowledge system for reuse, learning, and contribution evidence.</p>
      </div>

      <Card className="space-y-3 p-4">
        <CardTitle>Search & filters</CardTitle>
        <form action="/knowledge" method="get" className="grid gap-2 md:grid-cols-5">
          ...
          <div className="md:col-span-5 flex gap-2">
            ...
            <a className="inline-flex items-center text-xs underline" href="/knowledge">
              Reset
            </a>
          </div>
        </form>
      ...
      {canCreate ? (
        <Card className="space-y-3 p-4">
          <CardTitle>Create knowledge asset</CardTitle>
              ...
              <ul className="space-y-2 text-sm">
                {rows.map((a) => (
                  <li key={a.id} className="rounded-md border border-[hsl(var(--border))] px-3 py-2">
                    ...
                    <div className="text-xs text-[hsl(var(--muted))]">by {a.author.name} · {a.project?.name ?? "General"} · reused {a.reuseCount} times</div>
